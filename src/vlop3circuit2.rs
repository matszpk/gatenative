...
const MTUAREA_COMBS: [u8; 44] = [
    ...
    0b0011101, // (R,C1,C00,C01)
    0b1111000, // (C00,C01,C10,C11)
    0b1110100, // (C1,C01,C10,C11)
    0b1101100, // (C1,C00,C10,C11)
    0b1011010, // (C0,C00,C01,C11)
    0b0111010, // (C0,C00,C01,C10)
    0b1110000, // (C01,C10,C11)
    0b1101000, // (C00,C10,C11)
    0b1011000, // (C00,C01,C11)
    0b0111000, // (C00,C01,C10)
    0b1100100, // (C1,C10,C11)
    0b0011010, // (C0,C00,C01)
    0b1100010, // (C0,C10,C11)
    0b0011100, // (C1,C00,C01)
    0b0000111, // (R,C0,C1)
    0b1100000, // (C10,C11)
    ...
    fn improve_and_optimize_and_gen_lop3nodes(
        ...
        for i in &value_tbl_choosen[p..] {
            ...
            if current_cost < best_choice.0 {
                let mut enabled_nodes: Vec<Vec<(bool, u8)>> = lop3node_variants
                    .iter()
                    .map(|v| vec![(true, 0u8); v.len()])
                    .collect::<Vec<_>>();
                let mut found = true;
                    ...
                    for (vi, variant) in variants.iter().enumerate() {
                        for arg in &variant.args {
                            if let Some(p) = preferred_nodes.iter().position(|x| *arg == *x) {
                                if (i & (1 << p)) == 1 {
                                    enabled_nodes[tni][vi].1 = 1 << p;
                                } else {
                                    // if outside nodes in MTUarea comb
                                    enabled_nodes[tni][vi].0 = false;
                                }
                            }
                        }
                    }
