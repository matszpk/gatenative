...
fn test_parseq_mapper_all_data_holder() {
    for seq_doubling in [false, true] {
        ...
        let mut builder = ParSeqMapperBuilder::new(par_builder, seq_builders, seq_doubling);
        let seq_num = builder.seq_builder_num();
        let circuit =
...
fn test_parseq_mapper_builder_and_exec() {
    for (do_use_seq, seq_doubling) in [(false, false), (true, false), (true, true)] {
