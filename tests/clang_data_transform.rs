...
"##,
    ...
    for (idx = 0; idx < n; idx++) {
    unsigned int temp[128];
    const unsigned int* inelem = input + 256*idx;
    const size_t tpidx = 0;
    uint32x4_t* outelem = output + 10*idx;
    uint32x4_t v0;
    uint32x4_t v1;
    uint32x4_t v2;
    uint32x4_t v3;
...
"##,
    ...
    let mut dt = CLANG_DATA_TRANSFORM_U32.data_transform(128);
    dt.output_transform(
        "blable",
        10,
        64,
    ...
    for (idx = 0; idx < n; idx++) {
    unsigned int temp[32];
    const size_t tpidx = idx & 3;
    const uint32_t* inelem = input + 10*(idx & ~(size_t)3) + tpidx;
...
"##,
    ...
    let mut dt = CLANG_DATA_TRANSFORM_U32.data_transform(32 * 7);
    dt.output_transform(
        "blable",
        10,
        64,
    ...
    unsigned int temp[32];
    const size_t tpidx = idx % 7;
    const uint32_t* inelem = input + 10*(idx - tpidx) + tpidx;
    unsigned int* outelem = output + 448*idx;
    uint32_t v0;
    uint32_t v1;
    uint32_t v2;
    uint32_t v3;
...
"##,
    ...
    let mut dt = CLANG_DATA_TRANSFORM_INTEL_SSE2.data_transform(128);
    dt.output_transform(
        "blable",
        10,
        64,
    ...
    unsigned int temp[128];
    const size_t tpidx = 0;
    const __m128i* inelem = input + 10*idx;
    unsigned int* outelem = output + 256*idx;
    __m128i v0;
    __m128i v1;
    __m128i v2;
    ...
    __m128i v4;
    __m128i v5;
    v0 = _mm_loadu_si128((const __m128i*)&inelem[7]);
    v1 = _mm_loadu_si128((const __m128i*)&inelem[2]);
    v2 = _mm_loadu_si128((const __m128i*)&inelem[9]);
    v3 = _mm_loadu_si128((const __m128i*)&inelem[5]);
    v4 = _mm_loadu_si128((const __m128i*)&inelem[8]);
    v5 = _mm_loadu_si128((const __m128i*)&inelem[1]);
    OUTPUT_TRANSFORM_B7(temp, v0,v1,v2,v3,zero,v4,v5);
    for (k = 0; k < 128; k++)
        outelem[256*tpidx + 2*k + 0] = temp[k];
    v0 = _mm_loadu_si128((const __m128i*)&inelem[4]);
    v1 = _mm_loadu_si128((const __m128i*)&inelem[6]);
    v2 = _mm_loadu_si128((const __m128i*)&inelem[0]);
    v3 = _mm_loadu_si128((const __m128i*)&inelem[3]);
    OUTPUT_TRANSFORM_B6(temp, v0,v1,v2,zero,zero,v3);
    for (k = 0; k < 128; k++)
...
"##,
    ...
    let mut dt = CLANG_DATA_TRANSFORM_INTEL_SSE2.data_transform(64);
    dt.output_transform(
        "blable",
        10,
        64,
    ...
    const size_t tpidx = 0;
    const uint64_t* inelem = input + 10*idx;
    unsigned int* outelem = output + 128*idx;
    uint64_t v0;
    uint64_t v1;
    uint64_t v2;
    uint64_t v3;
    ...
    OUTPUT_TRANSFORM_B7(temp, v0,v1,v2,v3,zero,v4,v5);
    for (k = 0; k < 64; k++)
        outelem[128*tpidx + 2*k + 0] = temp[k];
    v0 = inelem[4];
    v1 = inelem[6];
    v2 = inelem[0];
    v3 = inelem[3];
    OUTPUT_TRANSFORM_B6(temp, v0,v1,v2,zero,zero,v3);
    for (k = 0; k < 64; k++)
        outelem[128*tpidx + 2*k + 1] = temp[k];
    }
}
"##,
        String::from_utf8(dt.out()).unwrap()
